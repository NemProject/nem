...
These values do not need to be fetched before every transfer.
...
The snippet then creates a <ser:TransferTransactionV2> from the transaction's descriptor, including a `mosaics` field
that accepts up to 10 entries.
...
* **Definition and supply** (line 6): The mosaic `company:token`, with its divisibility (`0`) and current supply
    (`1000000`), fetched to convert the quantity to atomic units and to calculate the fee.

* **Transaction fee** (line 16): `350000` atomic units (`0.35` XEM), derived from the mosaic's supply, divisibility, and
    the quantity sent.

* **Amount as multiplier** (line 19): With mosaics attached, `amount` is `1000000` (a factor of one) rather than an
    amount of XEM.

* **Mosaics array** (lines 20-27): The mosaics included in the transfer, here a single entry with a quantity of `100`.
...
| [Build a mosaic transfer](#building-the-transaction)                        | <dy:NemFacade.createTransactionFromTypedDescriptor>, <ser:TransferTransactionV2> |
