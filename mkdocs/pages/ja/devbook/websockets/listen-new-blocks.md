...
このチュートリアルでは、チャネルをサブスクライブして、届いた更新を表示する方法を説明します。

!!! note "ポーリングによる方法"

    ポーリングを使う方法については、[チェーンの高さと不可逆高さを照会する](../chain/chain-heights.md) チュートリアルを参照してください。
...
サブスクリプションには `id`（`id-0`）を付けます。終了時のサブスクライブ解除に使います。
...
受信する各メッセージの本文は、[Block](../reference/rest/nem.md#model/Block) スキーマに従う新しいブロックです。
...
!!! warning "新しいブロックはまだ確定していません"
    ...
    [チェーンの高さと不可逆高さを照会する](../chain/chain-heights.md) チュートリアルでは、不可逆高さの計算方法を説明しています。
