...
prefixes = tuple(mkdocs_gen_files.editor.FilesEditor.current().config["extra"]["nem"]["java-sdk"]["include-prefixes"])
for f in mkdocs_gen_files.editor.FilesEditor.current().files:
	if f.src_uri.startswith("devbook/reference/java/"):
		...
		match = re.search(r'^# <code .*?></code> ([^\s]*)', f.content_bytes.decode("utf-8"))
		if match:
