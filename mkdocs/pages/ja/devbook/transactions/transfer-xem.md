...
このヘルパーは JSON ペイロードと、人間が読めるラベルを受け取ります。
ラベルはログメッセージでのみ使用されるため、複数のトランザクションをアナウンスするチュートリアルでも同じヘルパーを再利用できます。
...
アナウンス用ヘルパーと同様に、このヘルパーもラベルを受け取るため、複数のトランザクションをアナウンスするチュートリアルでも出力が分かりやすくなります。
...
トランザクションが未承認の間、エンドポイントは HTTP エラーを返します。コードは 1 秒待ってから再試行し、最大 120 回（約 2 分）繰り返します。
...
* **トランザクションハッシュ**（17 行目）: ネットワーク上でトランザクションを一意に識別するハッシュ。
* **アナウンス結果**（19 行目）: `SUCCESS` はノードがトランザクションを未承認プールに受け入れたことを意味します。
* **承認**（33 行目）: トランザクションがブロック `626588` に含まれています。
