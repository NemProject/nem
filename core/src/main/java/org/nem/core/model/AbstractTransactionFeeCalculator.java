...
public abstract class AbstractTransactionFeeCalculator implements TransactionFeeCalculator {
	...
	protected Amount calculateMinimumFeeImpl(final TransferTransaction transaction) {
		final long messageFee = null == transaction.getMessage() ? 0 : transaction.getMessageLength() / 32 + 1;
	...
	private static long calculateMosaicTransferFee(final Amount amount, final Mosaic mosaic, final MosaicFeeInformation information) {
	...
	public boolean isFeeValid(Transaction transaction, BlockHeight blockHeight) {
		...
		// 1000 xem is the maximum fee that helps push a transaction into the cache
