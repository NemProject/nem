...
public class MathUtils {
	...
	/**
	 * Converts a 2^25.5 bit representation to a BigInteger. Value: 2^EXPONENTS[0] * t[0] + 2^EXPONENTS[1] * t[1] + ... + 2^EXPONENTS[9] *
	 * t[9]
	 *
	...
	/**
	 * Calculates (a * b + c) mod group order and returns the result. a, b and c are given in 2^8 bit representation.
	 *
	 ...
	 */
	public static Ed25519EncodedFieldElement multiplyAndAddModGroupOrder(final Ed25519EncodedFieldElement a,
	...
	/**
	 * Gets a random group element in P3 coordinates. It's NOT guaranteed that the created group element is a multiple of the base point.
	 *
	...
	/**
	 * Gets a random encoded group element. It's NOT guaranteed that the created encoded group element is a multiple of the base point.
	 *
	...
	/**
	 * Creates a group element from a byte array. Bit 0 to 254 are the affine y-coordinate, bit 255 is the sign of the affine x-coordinate.
	 *
	...
	public static Ed25519GroupElement toGroupElement(final byte[] bytes) {
		...
		return Ed25519GroupElement.p3(toFieldElement(x), toFieldElement(y), Ed25519Field.ONE,
	...
	/**
	 * Calculates and returns the square root of a fraction of u and v. The sign is unpredictable.
	 *
	...
	private static BigInteger getSqrtOfFraction(final BigInteger u, final BigInteger v) {
		final BigInteger tmp = u.multiply(v.pow(7)).modPow(BigInteger.ONE.shiftLeft(252).subtract(new BigInteger("3")), Ed25519Field.P)
				.mod(Ed25519Field.P);
		return tmp.multiply(u).multiply(v.pow(3)).mod(Ed25519Field.P);
	}

	/**
	 * Converts a group element from one coordinate system to another. This method is a helper used to test various methods in
	 * Ed25519GroupElement.
	 *
	...
	public static Ed25519GroupElement toRepresentation(final Ed25519GroupElement g, final CoordinateSystem newCoordinateSystem) {
		...
		switch (newCoordinateSystem) {
			...
			case CACHED:
				return Ed25519GroupElement.cached(toFieldElement(y.add(x).mod(Ed25519Field.P)),
						toFieldElement(y.subtract(x).mod(Ed25519Field.P)), Ed25519Field.ONE,
						toFieldElement(D.multiply(new BigInteger("2")).multiply(x).multiply(y).mod(Ed25519Field.P)));
			case PRECOMPUTED:
				return Ed25519GroupElement.precomputed(toFieldElement(y.add(x).mod(Ed25519Field.P)),
	...
	/**
	 * Adds two group elements and returns the result in P3 coordinate system. It uses BigInteger arithmetic and the affine coordinate
	 * system. This method is a helper used to test the projective group addition formulas in Ed25519GroupElement.
	 *
	...
	public static Ed25519GroupElement addGroupElements(final Ed25519GroupElement g1, final Ed25519GroupElement g2) {
		...
		final BigInteger x3 = g1x.multiply(g2y).add(g2x.multiply(g1y)).multiply(BigInteger.ONE.add(dx1x2y1y2).modInverse(Ed25519Field.P))
				.mod(Ed25519Field.P);
	...
	/**
	 * Doubles a group element and returns the result in the P3 coordinate system. It uses BigInteger arithmetic and the affine coordinate
	 * system. This method is a helper used to test the projective group doubling formula in Ed25519GroupElement.
	 *
	 * @param g The group element.
	 * @return g+g.
	 */
	 ...
	 * @return The resulting group element.
	 */
	public static Ed25519GroupElement doubleScalarMultiplyGroupElements(final Ed25519GroupElement g1, final Ed25519FieldElement f1,
			final Ed25519GroupElement g2, final Ed25519FieldElement f2) {
	...
	public static Signature sign(final KeyPair keyPair, final byte[] data) {
		...
		a[31] |= 0x40;
		a[0] &= 0xF8;
		final Ed25519EncodedFieldElement r = new Ed25519EncodedFieldElement(Hashes.sha3_512(Arrays.copyOfRange(hash, 32, 64), data));
		...
		final Ed25519EncodedFieldElement h = new Ed25519EncodedFieldElement(
				Hashes.sha3_512(R.encode().getRaw(), keyPair.getPublicKey().getRaw(), data));
