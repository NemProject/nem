...
public class NxtGraphClusteringITCase {
    ...
    public void canPrintStakes() throws SQLException {
        ...
        HashMap<Long, Long> stakes = getStakes(transactionData);
        System.out.println(stakes);
    }

    @Test
    public void canPrintImportances() throws SQLException, IOException {
        final String options = "_" + PoiAccountInfo.MIN_HARVESTING_BALANCE.getNumNem() + "min_" + /*"100MinTrans_0.05transProb_" +*/ GraphConstants.MU + "mu_" + GraphConstants.EPSILON + "epsilon";
        ...
        PoiAccountState genesis = createAccountWithBalance(Address.fromEncoded("1739068987193023818"), 1000000000000000l);
...
//			}
    ...
    public void canQueryNxtTransactionTable() throws SQLException {
        ...
        // Act:
        String sql = "SELECT height, sender_id, recipient_id, amount FROM TRANSACTION WHERE height = 0 ORDER BY height ASC";
    ...
    public void nxtGraphClusteringTest() throws SQLException {
        ...
        List<GraphClusteringStrategy> clusterers = getClusterers();
    ...
    private SparseMatrix createNetOutlinkMatrix(List<NxtTransaction> transactionData) {
...
//        return String.format("jdbc:h2:%s/nxt", System.getProperty("user.home"));
    ...
    private List<NxtTransaction> getTransactionData(int startHeight, int stopHeight) throws SQLException {
    ...
    private List<NxtTransaction> createAccountsFromTransactions(int startHeight, int stopHeight) throws SQLException {
        final List<NxtTransaction> transactionData = new ArrayList<>(stopHeight - startHeight);
    ...
    private Connection openNxtDatabase(String path) {
        if (conn != null) {
            return conn;
        }
        try {
    ...
    private List<GraphClusteringStrategy> getClusterers() {
        List<GraphClusteringStrategy> clusterers = new ArrayList<>();
    ...
    private class NxtTransaction {
        ...
        private NxtTransaction(long height, long senderId, long recipientId, long amount) {
            this.height = height;
    ...
    private void addOutlink(final PoiAccountState a, final PoiAccountState b, final BlockHeight blockHeight, final long amount) {
        ...
        long aBalance = a.getWeightedBalances().getUnvested(blockHeight).getNumMicroNem() + a.getWeightedBalances().getVested(blockHeight).getNumMicroNem();
        if (aBalance < amount) {
