...
public class BlockScorer {
	...
	public BigInteger calculateTarget(final Block prevBlock, final Block block) {
		...
		long forgerBalance = block.getSigner().getBalance(prevBlock.getHeight(), new BlockHeight(Math.max(1, block.getHeight().getRaw() - BlockChain.ESTIMATED_BLOCKS_PER_DAY))).getNumNem();
