...
/**
 * Helper class for storing NEM globals that can be accessed by other core classes. <br>
 * This class should really be used sparingly!
 */
public class NemGlobals {
	private static final SetOnce<TransactionFeeCalculator> TRANSACTION_FEE_CALCULATOR = new SetOnce<>(
			new TransactionFeeCalculatorBeforeFork());

	private static final SetOnce<BlockChainConfiguration> BLOCK_CHAIN_CONFIGURATION = new SetOnce<>(
			new BlockChainConfigurationBuilder().build());
