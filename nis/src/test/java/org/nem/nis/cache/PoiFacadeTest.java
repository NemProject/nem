...
public abstract class PoiFacadeTest<T extends CopyableCache<T> & PoiFacade> {
	...
	public void findStateByAddressReturnsStateForAddress() {
		...
		final AccountStateRepository facade = this.createPoiFacade();
	...
	private static List<AccountState> createAccountStatesForRecalculateTests(final int numAccounts, final AccountStateRepository facade) {
	...
	public void getDebitPredicateEvaluatesAmountAgainstBalancesInAccountState() {
		// Arrange:
		final AccountStateRepository accountStateRepository = this.createPoiFacade();
