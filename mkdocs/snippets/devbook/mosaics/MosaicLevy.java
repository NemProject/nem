...
public final class MosaicLevy {
	...
	private void run() throws IOException, InterruptedException {
		...
		final String levyRecipient = System.getenv().getOrDefault(
			"LEVY_RECIPIENT",
			"TBULEAUG2CZQISUR442HWA6UAKGWIXHDABJVIPS4");
		final long levyFee = 1_000_000;
		final MosaicLevyDescriptor levy = new MosaicLevyDescriptor(
			MosaicTransferFeeType.ABSOLUTE,
			new Address(levyRecipient),
			new MosaicIdDescriptor(
				new NamespaceIdDescriptor("nem".getBytes(UTF_8)),
				"xem".getBytes(UTF_8)),
			new Amount(levyFee));
		System.out.println("Levy:");
		System.out.println("  Type: absolute");
		System.out.printf("  Recipient: %s%n", levyRecipient);
		System.out.println("  Mosaic: nem:xem");
		System.out.printf("  Fee: %d%n", levyFee);
		// [<step-2]
		...
		final MosaicDefinitionDescriptor mosaicDefinition =
			new MosaicDefinitionDescriptor(
				...
				"My tutorial mosaic with a levy".getBytes(UTF_8),
				List.of(
		...
		final JsonNode levyInfo = info.get("levy");
		final JsonNode levyMosaicId = levyInfo.get("mosaicId");
		System.out.println("Levy information:");
		System.out.printf("  Type: %s%n",
			1 == levyInfo.get("type").asInt()
				? "absolute" : "percentile");
		System.out.printf("  Recipient: %s%n",
			levyInfo.get("recipient").asText());
		System.out.printf("  Mosaic: %s:%s%n",
			levyMosaicId.get("namespaceId").asText(),
			levyMosaicId.get("name").asText());
		System.out.printf("  Fee: %s%n", levyInfo.get("fee").asText());
		// [<step-5]
	}
}
