...
public abstract class GraphClusteringITCase {
	...
	private class SensitivityTestHarness {
		...
		private void renderAsList() {
			...
			final List<String> keyNames = keys.stream()
					.map(GraphClusteringITCase::getFriendlyLabel)
					.collect(Collectors.toList());
		...
		private void renderAsTable(final Amount minHarvesterBalance) {
			this.renderAsTable(minHarvesterBalance, GraphClusteringITCase::getFriendlyLabel);
		}
	...
	protected static AccountState createAccountWithBalance(final Address address, final long blockHeight, final Amount amount) {
