...
この例では、まず環境変数 `PRIVATE_KEY` から 16 進数文字列として秘密鍵を取得します。
環境変数が設定されている場合は、その値を <dy:PrivateKey> オブジェクトに変換します。
設定されていない場合は、代わりに <dy:PrivateKey.random> を使って新しいランダム秘密鍵を生成します。

!!! warning "秘密鍵を安全に保管してください"
    秘密鍵があれば、アカウントとそこに保有されている資産を完全に管理できます。
    秘密鍵を失うと、アカウントに永久にアクセスできなくなります。
    他人が秘密鍵を入手すると、アカウントを管理できてしまいます。

    秘密鍵は決して他人と共有せず、必ず安全な場所に保管してください。
...
環境変数なしでプログラムを実行するたびに、異なるランダムアカウントが生成されます。
