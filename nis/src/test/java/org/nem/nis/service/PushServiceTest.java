...
public class PushServiceTest {
	...
	public void pushTransactionValidFailureIncrementsFailedExperience() {
		...
		ExceptionAssert.assertThrows(
			v -> context.service.pushTransaction(transaction, context.remoteNodeIdentity),
			IllegalArgumentException.class);

		// Assert:
		context.assertSingleUpdateExperience(NodeInteractionResult.FAILURE);
	...
	public void pushTransactionValidSuccessAcceptedFailureIncrementsFailedExperience() {
		// Assert:
		final TestContext context = assertPushTransactionExperienceChange(NodeInteractionResult.FAILURE);
		Mockito.verify(context.network, Mockito.times(0)).broadcast(Mockito.any(), Mockito.any());
	}
	...
	public void pushTransactionValidSuccessAcceptedNeutralDoesNotChangeExperience() {
		// Assert:
		final TestContext context = assertPushTransactionExperienceChange(NodeInteractionResult.NEUTRAL);
		Mockito.verify(context.network, Mockito.times(0)).broadcast(Mockito.any(), Mockito.any());
	}
	...
	public void pushTransactionValidSuccessAcceptedSuccessIncrementsSuccessExperienceAndBroadcasts() {
		// Assert:
		final TestContext context = assertPushTransactionExperienceChange(NodeInteractionResult.SUCCESS);
	...
	private static TestContext assertPushTransactionExperienceChange(final NodeInteractionResult result) {
		// Arrange:
		final TestContext context = new TestContext();
		final MockTransaction transaction = new MockTransaction(Utils.generateRandomAccount(), 12);
		transaction.sign();
	...
	public void pushBlockValidFailureIncrementsFailedExperience() {
		...
		Mockito.when(context.blockChain.checkPushedBlock(block)).thenReturn(NodeInteractionResult.FAILURE);

		// Act:
		ExceptionAssert.assertThrows(
				v -> context.service.pushBlock(block, context.remoteNodeIdentity),
				IllegalArgumentException.class);
	...
	public void pushBlockValidNeutralAcceptedFailureIncrementsFailedExperience() {
		// Assert:
		final TestContext context = assertPushBlockExperienceChange(NodeInteractionResult.NEUTRAL, NodeInteractionResult.FAILURE);
		Mockito.verify(context.network, Mockito.times(0)).broadcast(Mockito.any(), Mockito.any());
	}
	...
	public void pushBlockValidSuccessAcceptedFailureIncrementsFailedExperience() {
		// Assert:
		final TestContext context = assertPushBlockExperienceChange(NodeInteractionResult.SUCCESS, NodeInteractionResult.FAILURE);
		Mockito.verify(context.network, Mockito.times(0)).broadcast(Mockito.any(), Mockito.any());
	}
	...
	public void pushBlockValidNeutralAcceptedNeutralDoesNotChangeExperience() {
		// Assert:
		final TestContext context = assertPushBlockExperienceChange(NodeInteractionResult.NEUTRAL, NodeInteractionResult.NEUTRAL);
		Mockito.verify(context.network, Mockito.times(0)).broadcast(Mockito.any(), Mockito.any());
	}
	...
	public void pushBlockValidSuccessAcceptedNeutralDoesNotChangeExperience() {
		// Assert:
		final TestContext context = assertPushBlockExperienceChange(NodeInteractionResult.SUCCESS, NodeInteractionResult.NEUTRAL);
		Mockito.verify(context.network, Mockito.times(0)).broadcast(Mockito.any(), Mockito.any());
	}
	...
	public void pushBlockValidNeutralAcceptedSuccessIncrementsSuccessExperienceAndBroadcasts() {
		// Assert:
		assertPushBlockAcceptedSuccessIncrementsSuccessExperienceAndBroadcasts(NodeInteractionResult.NEUTRAL);
	}

	@Test
	public void pushBlockValidSuccessAcceptedSuccessIncrementsSuccessExperienceAndBroadcasts() {
		// Assert:
		assertPushBlockAcceptedSuccessIncrementsSuccessExperienceAndBroadcasts(NodeInteractionResult.SUCCESS);
	}

	private static void assertPushBlockAcceptedSuccessIncrementsSuccessExperienceAndBroadcasts(
			final NodeInteractionResult validResult) {
		// Assert:
		final TestContext context = assertPushBlockExperienceChange(validResult, NodeInteractionResult.SUCCESS);
	...
	private static TestContext assertPushBlockExperienceChange(
			final NodeInteractionResult validResult,
			final NodeInteractionResult acceptedResult) {
		// Arrange:
		final TestContext context = new TestContext();
	...
	public void pushCanFailForUnidentifiedNode() {
		...
		// Act:
		ExceptionAssert.assertThrows(
				v -> context.service.pushTransaction(transaction, null),
				IllegalArgumentException.class);
	...
	public void pushCanSucceedForUnidentifiedNode() {
		...
		Mockito.when(context.foraging.processTransaction(transaction)).thenReturn(NodeInteractionResult.SUCCESS);
	...
	private static class TestContext {
		...
		public void assertSingleUpdateExperience(final NodeInteractionResult expectedResult) {
			Mockito.verify(this.network, Mockito.times(1)).updateExperience(this.remoteNode, expectedResult);
