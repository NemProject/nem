...
public class SmartTileBagTest {
	...
	@Test
	public void cannotCreateSmartTileBagIfSmartTileCollectionContainsDuplicateMosaicId() {
		// Arrange:
		final SmartTile smartTile = new SmartTile(Utils.createMosaicId(5), Quantity.fromValue(123));

		// Assert:
		ExceptionAssert.assertThrows(v -> new SmartTileBag(Arrays.asList(smartTile, smartTile)), IllegalArgumentException.class);
	}

	// region isEmpty

	@Test
	public void isEmptyReturnsTrueIfBagIsEmpty() {
		// Act:
		final SmartTileBag bag = new SmartTileBag(Collections.emptyList());

		// Assert:
		Assert.assertThat(bag.isEmpty(), IsEqual.equalTo(true));
	}

	@Test
	public void isEmptyReturnsFalseIfBagIsNotEmpty() {
		// Act:
		final SmartTile smartTile = new SmartTile(Utils.createMosaicId(5), Quantity.fromValue(123));
		final SmartTileBag bag = new SmartTileBag(Collections.singletonList(smartTile));

		// Assert:
		Assert.assertThat(bag.isEmpty(), IsEqual.equalTo(false));
	}
